...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("******the first leader is %d******", leader1)

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	DPrintf("******the first leader disconnect******")
	cfg.checkOneLeader()
	...
	DPrintf("******the first leader reconnect******")

	cfg.connect(leader1)
	//DPrintf("bug!!!!!!!!!!!!")
	leader2 := cfg.checkOneLeader()

	DPrintf("******the second leader is %d******", leader2)

	// if there's no quorum, no new leader should
	// be elected.
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("******majority severs disconnect******")
	time.Sleep(2 * RaftElectionTimeout)
	...
	DPrintf("******server %d reconnect, 2 servers now******", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("******3 servers now******")
	cfg.checkOneLeader()
